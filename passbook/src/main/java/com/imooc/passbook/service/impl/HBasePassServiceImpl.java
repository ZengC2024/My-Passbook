...
/**
 * <h1> Pass Template to HBase 服务</h1>*/
@Service
@Slf4j
public class HBasePassServiceImpl implements IHBasePassService {
    ...
    public boolean dropPassTemplateToHBase(PassTemplate passTemplate) {
        ...
        try{
            if(hbaseTemplate.getConnection().getTable(TableName.valueOf(
                    Constants.PassTemplateTable.TABLE_NAME)).exists(new Get(Bytes.toBytes(rowKey)))){
